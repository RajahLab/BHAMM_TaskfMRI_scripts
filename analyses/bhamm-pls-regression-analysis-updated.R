...
bhamm <- read_sav("~/BHAMM/PLS/BHAM_N72_MA_31Pre41Post_MRIsample_CorrectedFA_RT.sav") %>% 
  clean_names() %>%
...
# MODEL COMPARISON FOR LINEAR MIXED MODELS ------------------------------------------
# Added by Annalise LaPlume
# Fit hierarchical linear mixed models with and without age as a predictor
# Compare models to see which offers best fit to data

# Model comparison for pre meno group

# Fit nested lmms
lmm_pre_task <- lmerTest::lmer(cs_rate ~ task + (1|id), data = pre_long)
lmm_pre_task_age <- lmerTest::lmer(cs_rate ~ task * s2_age_std + (1|id), data = pre_long)
# Compare models
anova(lmm_pre_task, lmm_pre_task_age)

# Model comparison for post meno group

# Fit nested lmms
lmm_post_task <- lmerTest::lmer(cs_rate ~ task + (1|id), data = post_long)
lmm_post_task_age <- lmerTest::lmer(cs_rate ~ task * s2_age_std + (1|id), data = post_long)
# Compare models
anova(lmm_post_task, lmm_post_task_age)
